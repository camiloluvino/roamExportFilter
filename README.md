...
- **Copiar UIDs**: Opción "🔗 Copiar UIDs" en el footer para copiar referencias en formato `((uid))` de todos los bloques seleccionados (o referencias `[[Página]]` en la pestaña de Páginas). Soporta la copia con estructura jerárquica de ancestros.
- **Incluir Ancestros**: Opción interactiva "Incluir ancestros (copiar/exportar)" para adjuntar de forma estructurada los bloques padres del bloque seleccionado al copiar texto, copiar UIDs o exportar archivos (Markdown/EPUB), preservando el contexto jerárquico.
...
- **Ruta de Contexto**: Las exportaciones por ramas incluyen un registro de la **Jerarquía original** (bloques padres) en formato de cita al inicio del archivo, permitiendo conservar el contexto sin afectar la indentación del contenido principal (también soportado como estructura integrada en el documento).
